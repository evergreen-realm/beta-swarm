...
class Neo4jBrain:
    ...
    def store_evolution(self, project_id: str, learnings: dict):
        """Store post-project evolution learnings into the global graph."""
        import json
        content = json.dumps(learnings, default=str)[:3000]
        query = """
        MERGE (p:Project {id: $project_id})
        CREATE (e:Evolution {content: $content, timestamp: timestamp()})
        CREATE (p)-[:EVOLVED_WITH]->(e)
        """
        try:
            with self.driver.session() as session:
                session.run(query, project_id=project_id, content=content)
            return {"status": "success"}
        except Exception as e:
            logger.error(f"Error storing evolution for {project_id}: {e}")
            return {"status": "error", "message": str(e)}


# Backward-compat alias used by s5_levelcode.py and orchestrator.py
Neo4jManager = Neo4jBrain
